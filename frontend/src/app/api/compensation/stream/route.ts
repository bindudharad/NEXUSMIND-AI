...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Compensation stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Compensation stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/compensation/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing compensation stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Compensation stream failed\"}\n\n", {
      status: 500,
