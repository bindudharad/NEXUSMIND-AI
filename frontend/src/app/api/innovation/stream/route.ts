...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Innovation stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Innovation stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/innovation/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing innovation stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Innovation stream failed\"}\n\n", {
      status: 500,
