...
async function token() {
  ...
  if (!response.ok) throw new Error("Digital twin proxy authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/intelligence/digital-twin/simulate`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(toCamel(data), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Digital twin simulation failed" }, { status: 500 });
  }
}
