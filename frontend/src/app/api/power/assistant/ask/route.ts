...
async function token() {
  ...
  if (!response.ok) throw new Error("Manager assistant authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/power/assistant/ask`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Manager assistant failed" }, { status: 500 });
  }
}
