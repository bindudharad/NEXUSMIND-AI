...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("People intelligence stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("People intelligence stream authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/genai/hr/stream`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok || !response.body) throw new Error("Missing people intelligence stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"People intelligence stream failed\"}\n\n", {
      status: 500,
