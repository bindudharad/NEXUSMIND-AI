...
async function token() {
  ...
  if (!response.ok) throw new Error("Client relationship assistant authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/clients/relationship/assistant`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Client relationship assistant failed" }, { status: 500 });
  }
}
