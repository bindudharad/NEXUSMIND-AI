...
async function token() {
  ...
  if (!response.ok) throw new Error("Anomaly feedback authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/anomalies/feedback`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Anomaly feedback failed" }, { status: 500 });
  }
}
