...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Innovation assistant backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Innovation assistant authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/innovation/assistant`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Innovation assistant failed" }, { status: 500 });
  }
}
