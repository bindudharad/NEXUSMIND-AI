...
async function token() {
  ...
  if (!response.ok) throw new Error("Smart suggestion authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/suggestions/feed`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Smart suggestion feed failed" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ detail: "Smart suggestion generation failed" }, { status: 500 });
  }
}
