...
async function token() {
  ...
  if (!response.ok) throw new Error("Attrition stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/attrition/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.body) throw new Error("Missing attrition stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Attrition stream failed\"}\n\n", {
      status: 500,
