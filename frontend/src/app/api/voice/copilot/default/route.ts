...
async function token() {
  ...
  if (!response.ok) throw new Error("Voice copilot authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/voice/copilot/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Voice copilot default command failed" }, { status: 500 });
  }
}
