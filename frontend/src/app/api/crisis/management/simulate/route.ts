...
async function token() {
  ...
  if (!response.ok) throw new Error("Crisis simulation authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/crisis/management/simulate`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Crisis simulation failed" }, { status: 500 });
  }
}
