...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Anomaly stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Anomaly stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/anomalies/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing anomaly stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Anomaly stream failed\"}\n\n", {
      status: 500,
