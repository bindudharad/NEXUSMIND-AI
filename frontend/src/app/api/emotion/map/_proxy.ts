...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Company emotion map backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Company emotion map authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing company emotion map stream");
