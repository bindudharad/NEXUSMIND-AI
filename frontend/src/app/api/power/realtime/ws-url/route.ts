const API_BASE_URL = process.env.API_BASE_URL ?? process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://127.0.0.1:8000/api/v1";
const DEMO_EMAIL = process.env.DEMO_EMAIL ?? "ceo@nexusmind.ai";
const DEMO_PASSWORD = process.env.DEMO_PASSWORD ?? "nexusmind-demo";

async function token() {
  const response = await fetch(`${API_BASE_URL}/auth/login`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email: DEMO_EMAIL, password: DEMO_PASSWORD }),
    cache: "no-store",
  });
  if (!response.ok) throw new Error("Power WebSocket authentication failed");
  return ((await response.json()) as { access_token: string }).access_token;
}

export async function GET() {
  try {
    const accessToken = await token();
    const apiUrl = new URL(API_BASE_URL);
    const protocol = apiUrl.protocol === "https:" ? "wss:" : "ws:";
    const basePath = apiUrl.pathname.replace(/\/api\/v1\/?$/, "");
    const wsUrl = `${protocol}//${apiUrl.host}${basePath}/api/v1/power/realtime/ws?token=${encodeURIComponent(accessToken)}`;

    return Response.json({
      url: wsUrl,
      auth: "jwt-query-token",
      expiresInSeconds: 60 * 60,
    });
  } catch {
    return Response.json({ detail: "Power realtime WebSocket URL failed" }, { status: 500 });
  }
}
