...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Benchmarking backend request failed");
}
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/benchmarks/companies/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await readJson(response)), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Benchmarking analysis failed" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/benchmarks/companies/analyze`, {
      method: "POST",
      headers: {
