...
async function token() {
  ...
  if (!response.ok) throw new Error("Digital twin snapshot authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/intelligence/digital-twin/company`, {
    ...
    return NextResponse.json({ detail: "Digital twin snapshot failed" }, { status: 500 });
  }
}
