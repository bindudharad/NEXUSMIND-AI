...
async function token() {
  ...
  if (!response.ok) throw new Error("Smart suggestion stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/suggestions/stream`, {
    ...
    return new Response("event: error\ndata: {\"detail\":\"Smart suggestion stream failed\"}\n\n", {
      status: 500,
