...
async function token() {
  ...
  if (!response.ok) throw new Error("Business Prediction authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing Business Prediction stream");
