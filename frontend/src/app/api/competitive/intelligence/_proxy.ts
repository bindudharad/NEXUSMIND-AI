...
async function token() {
  ...
  if (!response.ok) throw new Error("Competitive intelligence authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing competitive intelligence stream");
