...
async function token() {
  ...
  if (!response.ok) throw new Error("Autonomous Workflow authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing Autonomous Workflow stream");
