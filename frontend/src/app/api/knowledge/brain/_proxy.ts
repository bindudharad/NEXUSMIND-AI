...
export async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Company Brain backend request failed");
}
...
export async function token() {
  ...
  if (!response.ok) throw new Error("Company Brain authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing Company Brain stream");
