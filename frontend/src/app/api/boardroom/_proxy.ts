...
async function requestToken() {
  ...
  if (!response.ok) throw new Error("Boardroom command center authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error(`Missing boardroom command-center stream from ${API_BASE_URL}`);
