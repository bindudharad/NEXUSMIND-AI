...
async function token() {
  ...
  if (!response.ok) throw new Error("Scenario simulation authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/intelligence/scenario/simulate`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Scenario simulation failed" }, { status: 500 });
  }
}
