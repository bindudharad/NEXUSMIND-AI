...
async function token() {
  ...
  if (!response.ok) throw new Error("Complete platform authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/platform/operating-system`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Complete platform verification failed" }, { status: 500 });
  }
}
