...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Learning stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Learning stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/learning/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing learning stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Learning stream failed\"}\n\n", {
      status: 500,
