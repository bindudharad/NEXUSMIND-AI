...
async function token() {
  ...
  if (!response.ok) throw new Error("Wellness authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/wellness/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Mental wellness analysis failed" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/wellness/analyze`, {
      method: "POST",
      headers: {
