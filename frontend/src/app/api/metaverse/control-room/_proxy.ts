...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Enterprise Metaverse backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Enterprise Metaverse authentication failed");
...
export async function proxyJson(path: string, init: RequestInit = {}) {
  try {
    ...
    return NextResponse.json({ detail: error instanceof Error ? error.message : "Enterprise Metaverse request failed" }, { status: 500 });
  }
}
...
export async function proxyStream(path: string) {
  try {
    ...
    if (!response.body) throw new Error("Missing Enterprise Metaverse stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Enterprise Metaverse stream failed\"}\n\n", {
      status: 500,
