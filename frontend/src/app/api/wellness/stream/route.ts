...
async function token() {
  ...
  if (!response.ok) throw new Error("Wellness stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/wellness/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.body) throw new Error("Missing wellness stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Mental wellness stream failed\"}\n\n", {
      status: 500,
