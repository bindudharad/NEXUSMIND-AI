...
async function token() {
  ...
  if (!response.ok) throw new Error("Self-learning demo authentication failed");
...
export async function POST() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/self-learning/demo`, {
      method: "POST",
    ...
    return NextResponse.json({ detail: "Self-learning demo failed" }, { status: 500 });
  }
}
