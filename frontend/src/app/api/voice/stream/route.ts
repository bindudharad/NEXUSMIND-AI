...
async function token() {
  ...
  if (!response.ok) throw new Error("Voice stress stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/voice/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.body) throw new Error("Missing voice stress stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Voice stress stream failed\"}\n\n", {
      status: 500,
