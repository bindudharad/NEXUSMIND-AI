...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Project failure stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Project failure stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/projects/failure/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing project failure stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Project failure stream failed\"}\n\n", {
      status: 500,
