...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Project failure backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Project failure authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/projects/failure/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await readJson(response)), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Project failure prediction failed" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/projects/failure/predict`, {
      method: "POST",
      headers: {
