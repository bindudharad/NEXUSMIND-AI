...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Productivity stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Productivity stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/productivity/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing productivity stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Productivity leakage stream failed\"}\n\n", {
      status: 500,
