...
const TOKEN_CACHE_TTL_MS = Number(process.env.API_TOKEN_CACHE_TTL_MS ?? 5 * 60 * 1000);
...
async function token() {
  ...
  pendingTokenRequest = requestToken()
    .then((accessToken) => {
      cachedAccessToken = { value: accessToken, expiresAt: Date.now() + TOKEN_CACHE_TTL_MS };
      return accessToken;
    })
    .finally(() => {
...
async function requestToken() {
  ...
  if (!response.ok) throw new Error("Living Company Brain authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing Living Company Brain stream");
