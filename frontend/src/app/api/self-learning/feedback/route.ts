...
async function token() {
  ...
  if (!response.ok) throw new Error("Self-learning feedback authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/self-learning/feedback`, {
      method: "POST",
    ...
    return NextResponse.json({ detail: "Self-learning feedback failed" }, { status: 500 });
  }
}
