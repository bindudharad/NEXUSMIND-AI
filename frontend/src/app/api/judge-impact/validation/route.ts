...
async function token() {
  ...
  if (!response.ok) throw new Error("Judge impact authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/judge-impact/validation`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Judge impact validation failed" }, { status: 500 });
  }
}
