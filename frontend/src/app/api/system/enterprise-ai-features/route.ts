...
export async function GET() {
  try {
    const response = await fetch(`${API_BASE_URL}/system/enterprise-ai-features`, { cache: "no-store" });
    const data = await response.json();
    return NextResponse.json(toCamel(data), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Enterprise AI feature verification failed" }, { status: 500 });
  }
}
