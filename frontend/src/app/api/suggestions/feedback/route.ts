...
async function token() {
  ...
  if (!response.ok) throw new Error("Smart suggestion feedback authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/suggestions/feedback`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Smart suggestion feedback failed" }, { status: 500 });
  }
}
