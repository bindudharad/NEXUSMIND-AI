...
async function token() {
  ...
  if (!response.ok) throw new Error("Organization optimizer authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/organization/optimizer/analyze`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Organization optimizer analysis failed" }, { status: 500 });
  }
}
