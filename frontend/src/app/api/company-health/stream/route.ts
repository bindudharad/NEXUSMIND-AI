...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Company health stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Company health stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/company-health/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing company health stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Company health stream failed\"}\n\n", {
      status: 500,
