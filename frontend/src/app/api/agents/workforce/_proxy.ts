...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Multi-agent workforce backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Multi-agent workforce authentication failed");
...
export async function proxyJson(path: string, init: RequestInit = {}) {
  try {
    ...
    return NextResponse.json({ detail: error instanceof Error ? error.message : "Multi-agent workforce request failed" }, { status: 500 });
  }
}
...
export async function proxyStream(path: string) {
  try {
    ...
    if (!response.body) throw new Error("Missing multi-agent workforce stream");
    ...
    const stream = response.body.pipeThrough(
      new TransformStream<Uint8Array, Uint8Array>({
        transform(chunk, controller) {
          ...
          buffer = events.pop() ?? "";
          for (const event of events) controller.enqueue(encoder.encode(`${toCamelEvent(event)}\n\n`));
        },
        flush(controller) {
    ...
    return new Response("event: error\ndata: {\"detail\":\"Multi-agent workforce stream failed\"}\n\n", {
      status: 500,
