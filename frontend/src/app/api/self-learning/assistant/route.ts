...
async function token() {
  ...
  if (!response.ok) throw new Error("Self-learning assistant authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/self-learning/assistant`, {
      method: "POST",
    ...
    return NextResponse.json({ detail: "Self-learning assistant failed" }, { status: 500 });
  }
}
