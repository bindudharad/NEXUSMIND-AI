...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Resource allocation stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Resource allocation stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/resources/allocation/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing resource allocation stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Resource allocation stream failed\"}\n\n", {
      status: 500,
