...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Knowledge loss stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Knowledge loss stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/knowledge/loss/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing knowledge loss stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Knowledge loss stream failed\"}\n\n", {
      status: 500,
