...
async function token() {
  ...
  if (!response.ok) throw new Error("Organization optimizer stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/organization/optimizer/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.body) throw new Error("Missing organization optimizer stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Organization optimizer stream failed\"}\n\n", {
      status: 500,
