...
async function token() {
  ...
  if (!response.ok) throw new Error("Scenario decision suite authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/intelligence/scenario/decision-suite`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Scenario decision suite failed" }, { status: 500 });
  }
}
