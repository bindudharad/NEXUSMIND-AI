...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Decision assistant stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Decision assistant stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/decisions/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing decision assistant stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Decision assistant stream failed\"}\n\n", {
      status: 500,
