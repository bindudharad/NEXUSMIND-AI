...
async function token() {
  ...
  if (!response.ok) throw new Error("Recommendation proxy authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/recommendations/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Recommendation generation failed" }, { status: 500 });
  }
}
