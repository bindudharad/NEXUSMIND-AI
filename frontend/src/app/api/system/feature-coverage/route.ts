...
export async function GET() {
  try {
    const response = await fetch(`${API_BASE_URL}/system/feature-coverage`, { cache: "no-store" });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Feature coverage audit failed" }, { status: 500 });
  }
}
