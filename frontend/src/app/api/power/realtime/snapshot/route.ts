...
async function token() {
  ...
  if (!response.ok) throw new Error("Power snapshot authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/power/realtime/snapshot`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Power realtime snapshot failed" }, { status: 500 });
  }
}
