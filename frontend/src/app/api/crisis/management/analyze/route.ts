...
async function token() {
  ...
  if (!response.ok) throw new Error("Crisis analysis authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/crisis/management/analyze`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Crisis analysis failed" }, { status: 500 });
  }
}
