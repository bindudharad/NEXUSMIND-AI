...
async function token() {
  ...
  if (!response.ok) throw new Error("Ultimate platform authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/ultimate-platform/verification`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Ultimate platform verification failed" }, { status: 500 });
  }
}
