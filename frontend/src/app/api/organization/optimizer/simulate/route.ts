...
async function token() {
  ...
  if (!response.ok) throw new Error("Organization simulation authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/organization/optimizer/simulate`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Organization simulation failed" }, { status: 500 });
  }
}
