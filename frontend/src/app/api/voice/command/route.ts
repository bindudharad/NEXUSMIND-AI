...
import { buildResilientVoiceCommandResponse } from "@/app/api/voice/_resilient";
...
async function token() {
  ...
  if (!response.ok) throw new Error("Voice command authentication failed");
...
export async function POST(request: Request) {
  ...
  if (!USE_LIVE_VOICE_BACKEND) return NextResponse.json(buildResilientVoiceCommandResponse(payload));
  try {
    const accessToken = await token();
    const response = await fetchWithTimeout(`${API_BASE_URL}/voice/command`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(buildResilientVoiceCommandResponse(payload));
  }
}
