...
async function token() {
  ...
  if (!response.ok) throw new Error("Strategic intelligence authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/strategic/enterprise`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Strategic intelligence failed" }, { status: 500 });
  }
}
