...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw new Error("Client satisfaction stream backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Client satisfaction stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/clients/satisfaction/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.ok || !response.body) throw new Error("Missing client satisfaction stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Client satisfaction stream failed\"}\n\n", {
      status: 500,
