...
async function token() {
  ...
  if (!response.ok) throw new Error("Power stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/power/realtime/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.body) throw new Error("Missing power stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Power realtime stream failed\"}\n\n", {
      status: 500,
