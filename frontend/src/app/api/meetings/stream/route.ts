...
async function token() {
  ...
  if (!response.ok) throw new Error("Meeting stream authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/meetings/stream`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    if (!response.body) throw new Error("Missing meeting stream");
    ...
    return new Response("event: error\ndata: {\"detail\":\"Meeting analysis stream failed\"}\n\n", {
      status: 500,
