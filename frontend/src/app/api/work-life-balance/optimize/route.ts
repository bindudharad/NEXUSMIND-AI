...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Work-life balance backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Work-life balance authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/work-life/balance/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await readJson(response)), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Work-life balance optimization failed" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/work-life/balance/optimize`, {
      method: "POST",
      headers: {
