...
async function token() {
  ...
  if (!response.ok) throw new Error("Recruiter impression authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/recruiter-impression/summary`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await response.json()), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Recruiter impression audit failed" }, { status: 500 });
  }
}
