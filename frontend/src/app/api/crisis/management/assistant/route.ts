...
async function token() {
  ...
  if (!response.ok) throw new Error("Crisis assistant authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/crisis/management/assistant`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Crisis assistant failed" }, { status: 500 });
  }
}
