...
async function backendFetch(input: string, init: RequestInit = {}) {
  ...
  throw lastError instanceof Error ? lastError : new Error("Learning backend request failed");
}
...
async function token() {
  ...
  if (!response.ok) throw new Error("Learning recommendation authentication failed");
...
export async function GET() {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/learning/default`, {
      headers: { Authorization: `Bearer ${accessToken}` },
      cache: "no-store",
    });
    return NextResponse.json(toCamel(await readJson(response)), { status: response.status });
  } catch {
    return NextResponse.json({ detail: "Learning recommendation failed" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const response = await backendFetch(`${API_BASE_URL}/learning/recommend`, {
      method: "POST",
      headers: {
