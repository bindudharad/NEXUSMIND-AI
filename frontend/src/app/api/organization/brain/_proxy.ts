...
async function token() {
  ...
  if (!response.ok) throw new Error("Organizational Brain authentication failed");
...
function toCamelEvent(eventBlock: string): string {
  return eventBlock
    .split("\n")
    .map((line) => {
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing Organizational Brain stream");
