...
async function token() {
  ...
  if (!response.ok) throw new Error("Organization assistant authentication failed");
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/organization/optimizer/assistant`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ detail: "Organization assistant failed" }, { status: 500 });
  }
}
