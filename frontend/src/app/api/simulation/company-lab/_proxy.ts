...
async function token() {
  ...
  if (!response.ok) throw new Error("Company Simulation Lab authentication failed");
...
export async function proxyStream(path: string) {
  ...
  if (!response.ok || !response.body) throw new Error("Missing Company Simulation Lab stream");
